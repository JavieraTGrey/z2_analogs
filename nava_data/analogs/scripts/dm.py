# Created on Oct 16th 11:48 AM
# @author : bnavarre
#
# This script takes the catalog of fluxes of emission lines of our galaxies
# and calculates a series of line ratios (Check below) together with the
# metallicity (or Oxygen abundance as indicator; 12 + log(O/H)) and N/O ratio.
# All these formulae and reasoning is taken from Pérez-Montero (2017).
...
#nar_cat = pd.read_csv(magePath+'results/em_narrow.csv')
#brd_cat = pd.read_csv(magePath+'results/em_broad.csv')

# Narrow and broad upper limit masks
#mask_ul = (nar_cat == 0.0) + (brd_cat == 0.0)

# New df accounting for upper limits
#el_cat = el_cat.where(~mask_ul, other=0.0)

# Assigning arrays & emission lines
galaxies = el_cat['galname'].to_numpy()

OII3727 = upy.uarray(el_cat['[OII]3726'].to_numpy(),
                     el_cat['[OII]3726_err'].to_numpy()) # oiid

OII3729 = upy.uarray(el_cat['[OII]3729'].to_numpy(),
                     el_cat['[OII]3729_err'].to_numpy()) # oiic

OIII4363 = upy.uarray(el_cat['[OIII]4363'].to_numpy(),
                      el_cat['[OIII]4363_err'].to_numpy()) # oiiic
...
Hb = upy.uarray(el_cat['Hb'].to_numpy(),
                el_cat['Hb_err'].to_numpy())

OIII4959 = upy.uarray(el_cat['[OIII]4959'].to_numpy(),
                      el_cat['[OIII]4959_err'].to_numpy()) # oiiib

OIII5007 = upy.uarray(el_cat['[OIII]5007'].to_numpy(),
                      el_cat['[OIII]5007_err'].to_numpy()) # oiiia

NII5755 = upy.uarray(el_cat['[NII]5755'].to_numpy(),
                     el_cat['[NII]5755_err'].to_numpy()) # niic

#OI6300 = upy.uarray(el_cat['[OI]6300'].to_numpy(),
#                    el_cat['[OI]6300_err'].to_numpy()) # oi

SIII6312 = upy.uarray(el_cat['[SIII]6312'].to_numpy(),
                      el_cat['[SIII]6312_err'].to_numpy()) # siiic

NII6548 = upy.uarray(el_cat['[NII]6548'].to_numpy(),
                     el_cat['[NII]6548_err'].to_numpy()) # niib

Ha = upy.uarray(el_cat['Ha'].to_numpy(),
                el_cat['Ha_err'].to_numpy())

NII6584 = upy.uarray(el_cat['[NII]6584'].to_numpy(),
                     el_cat['[NII]6584_err'].to_numpy()) # niia

SII6716 = upy.uarray(el_cat['[SII]6716'].to_numpy(),
                     el_cat['[SII]6716_err'].to_numpy()) # siib

SII6730 = upy.uarray(el_cat['[SII]6730'].to_numpy(),
                     el_cat['[SII]6730_err'].to_numpy()) # siia

OII7319 = upy.uarray(el_cat['[OII]7319'].to_numpy(),
                     el_cat['[OII]7319_err'].to_numpy()) # oiib

OII7330 = upy.uarray(el_cat['[OII]7330'].to_numpy(),
                     el_cat['[OII]7330_err'].to_numpy()) # oiia
...
def Tlow_ne(temp, path):
    ...
    for i in range(len(galaxies)):
        ...
        if temp == 'TO2':
            t_n = TO2(OII7330[i], OII7319[i], OII3729[i], OII3727[i], ne_toplot)
        elif temp == 'TS2':
            t_n = TS2(SII6730[i], SII6716[i], SII4076[i], SII4068[i], ne_toplot)
        ...
        fig.savefig(path + galaxies[i] + '.pdf', bbox_inches='tight')
        print()
        plt.close()
...
if __name__ == '__main__':
    ...
    for i in range(len(galaxies)):
        ...
        axs[0].set_ylabel('Electron density')
        axs[0].set_title(galaxies[i])
        axs[1].set_xlabel('T [OII]')
        axs[1].set_ylabel('|n - n(t(n))|')
        axs[1].axvline(upy.nominal_values(t_n)[index_min], ls='--')
        
        print('Value where functions cross: T[OII] =', t_n[index_min])
        print(type(upy.nominal_values(t_n)[index_min]))
        t_low_it.append(upy.nominal_values(t_n)[index_min])
        t_low_it_err.append(upy.std_devs(t_n)[index_min])
        
        plt.savefig('/home/benjamin/Documents/analogs/results/metals/n_vs_t2_'+galaxies[i]+'.pdf', bbox_inches='tight')
        
    
    # T[OII] and n_e determined simultaneously
    t_low_new = upy.uarray(t_low_it, t_low_it_err)
    n_e_new = ne(SII6730, SII6716, t_low_new)
    '''
    ...
    if mode == 'lines':
        ...
        ne_OII, tO2_ne = Tlow_ne('TO2', magePath + 'results/metals/n_vs_tO2_')
    ...
    if mode == 'lines':
        ...
        NO_ratio = log_NO(NII6584, OII3729, OII3727, t_low_OII)
    ...
    temden = pd.DataFrame({'Source': galaxies,
                          ...
                          # T(OIII) = T_high
                          't(OIII)': upy.nominal_values(t_high),
                          't(OIII)_err': upy.std_devs(t_high),
                          ...
                          # T(N2) = T_low 
                          't(NII)': upy.nominal_values(t_low_NII),
                          't(NII)_err': upy.std_devs(t_low_NII),
                          ...
                          't(OII)_OIII': upy.nominal_values(t_low_OII),
                          't(OII)_OIII_err': upy.std_devs(t_low_OII),
                          ...
                          't(OII)_ne': upy.nominal_values(tO2_ne),
                          't(OII)_ne_err': upy.std_devs(tO2_ne),
    ...
    metals = pd.DataFrame({'Source': galaxies,
                          ...
                          'N2': upy.nominal_values(N2),
                          'N2_err': upy.std_devs(N2),
                           
                          'R3': upy.nominal_values(R3),
                          'R3_err': upy.std_devs(R3),
                           
                          'R2': upy.nominal_values(R2),
                          'R2_err': upy.std_devs(R2),
                           
                          'R23': upy.nominal_values(R23),
                          'R23_err': upy.std_devs(R23),
                           
                          'O3N2': upy.nominal_values(O3N2),
                          'O3N2_err': upy.std_devs(O3N2),
                           
                          'R3N2': upy.nominal_values(R3N2),
                          'R3N2_err': upy.std_devs(R3N2),
                           
                          'N2O2': upy.nominal_values(N2O2),
                          'N2O2_err': upy.std_devs(N2O2),
                           
                          'O3O2': upy.nominal_values(O3O2),
                          'O3O2_err': upy.std_devs(O3O2),
                           
                          #'Ne3O2': upy.nominal_values(Ne3O2),
                          #'Ne3O2_err': upy.std_devs(Ne3O2),
                           
                          'S2': upy.nominal_values(S2),
                          'S2_err': upy.std_devs(S2),
                           
                          'O3S2': upy.nominal_values(O3S2),
                          'O3S2_err': upy.std_devs(O3S2),
                           
                          'N2S2': upy.nominal_values(N2S2),
                          'N2S2_err': upy.std_devs(N2S2)
    })
    
    # Saving
    temden.to_csv(magePath+'results/metals/temden_'+mode+'.csv', index=None)
    metals.to_csv(magePath+'results/metals/metals_'+mode+'.csv', index=None)
