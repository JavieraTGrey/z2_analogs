...
# Read the data
el_cat = pd.read_csv(magePath+'magE2024_master_Dcorr_parts.csv')
# Assigning arrays & emission lines
galaxies = el_cat['ID'].to_numpy()
mass = el_cat['mass'].to_numpy()
z_red = el_cat['z_red'].to_numpy()
z_blue = el_cat['z_blue'].to_numpy()
...
NE3868 = upy.uarray(el_cat['Ne3_3868_flux'].to_numpy(),
                    el_cat['Ne3_3868_fluxerr'].to_numpy())
OIII4363 = upy.uarray(el_cat['O3_4363_flux'].to_numpy(),
                      el_cat['O3_4363_fluxerr'].to_numpy())  # oiiic

SII4068 = upy.uarray(el_cat['S2_4068_flux'].to_numpy(),
                     el_cat['S2_4068_fluxerr'].to_numpy())  # siib

SII4076 = upy.uarray(el_cat['S2_4076_flux'].to_numpy(),
                     el_cat['S2_4076_fluxerr'].to_numpy())  # siia
...
OII7319 = upy.uarray(el_cat['O2_7320_flux'].to_numpy(),
                     el_cat['O2_7320_fluxerr'].to_numpy())  # oiib

OII7330 = upy.uarray(el_cat['O2_7331_flux'].to_numpy(),
                     el_cat['O2_7331_fluxerr'].to_numpy())  # oiia
...
Hg = upy.uarray(el_cat['H_gamma_flux'].to_numpy(),
                el_cat['H_gamma_fluxerr'].to_numpy())

ARIII7136 = upy.uarray(el_cat['Ar3_7136_flux'].to_numpy(),
                       el_cat['Ar3_7136_fluxerr'].to_numpy())
# =============================================================================
#
# Functions
#
...
def Tlow_ne(temp, path):
    ...
    determining the shape of both functions. Thus, by evaluating both
    expresions
    ...
    ------
    temp : str
        String indicating the temperature to use for the estimation.
        (e.g. 'TO2')
    ...
    for i in range(len(galaxies)):
        ...
        fig.savefig(magePath + f'images/n_vs_tO2_{galaxies[i]}_parts.pdf',
                    bbox_inches='tight')
        print()
        plt.close()
...
def N2S2_func(niia, siia, siib):
    assert (is_unc(niia))
    assert (is_unc(siia))
    assert (is_unc(siib))
    sii = siia + siib
    return upy.log10(niia / sii)


def NE3O2_func(neiii, oiia, oiib):
    assert (is_unc(neiii))
    assert (is_unc(oiia))
    assert (is_unc(oiib))
    oii = oiia + oiib
    return upy.log10(neiii / oii)


def NE3_func(neiii, hg):
    assert (is_unc(neiii))
    assert (is_unc(hg))
    return upy.log10(neiii / hg)


def RO2NE3_func(neiii, oiia, oiib, hg):
    assert (is_unc(neiii))
    assert (is_unc(oiia))
    assert (is_unc(oiib))
    assert (is_unc(hg))
    oii = oiia + oiib
    return upy.log10((neiii + oii) / hg)


def AR3_func(ariii, hg):
    assert (is_unc(ariii))
    assert (is_unc(hg))
    return upy.log10(ariii / hg)


def AR3O3_func(ariii, oiii):
    assert (is_unc(ariii))
    assert (is_unc(oiii))
    return upy.log10(ariii / oiii)
...
def ne(siia, siib, to3):
    ...
    assert (is_unc(to3))
    RS2 = siib / siia
    ne = 1e3 * ((RS2 * a0_t(to3)) + a1_t(to3)) / ((RS2 * b0_t(to3)) +
                                                  b1_t(to3))
...
if __name__ == '__main__':
    ...
    N2S2 = N2S2_func(NII6584, SII6730, SII6716)
    NE3O2 = NE3O2_func(NE3868, OII3729, OII3727)
    R = 0.47*(R2) + 0.88*(R3)
    NE3 = NE3_func(NE3868, Hg)
    RO2NE3 = RO2NE3_func(NE3868, OII3729, OII3727, Hg)
    AR3 = AR3_func(ARIII7136, Ha)
    AR3O3 = AR3O3_func(ARIII7136, OIII5007)
    ...
    if mode == 'lines':
        ne_SII, tS2_ne = Tlow_ne('TS2', magePath + 'results/metals/n_vs_tS2_parts_')
    ...
    metals = pd.DataFrame({'Source': galaxies,
                           ...
                           'R': upy.nominal_values(R),
                           'R_ERR': upy.std_devs(R),
                           'Ne3': upy.nominal_values(NE3),
                           'Ne3_ERR': upy.std_devs(NE3),
                           'RO2NE3': upy.nominal_values(RO2NE3),
                           'RO2NE3_ERR': upy.std_devs(RO2NE3),
                           'AR3': upy.nominal_values(AR3),
                           'AR3_ERR': upy.std_devs(AR3),
                           'AR3O3': upy.nominal_values(AR3O3),
                           'AR3O3_ERR': upy.std_devs(AR3O3)
                           })
    # Saving
    metals.to_csv(magePath + 'magE2024_lines_ratio1_parts.csv')
    temden.to_csv(magePath + f'ratios/temden_{mode}1_parts.csv', index=None)